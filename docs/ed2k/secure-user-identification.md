...
Every aMule client has a permanent **userhash** — a 128-bit value stored in [`~/.aMule/preferences.dat`](../manual/core/config-files/index.md#preferencesdat). The userhash is the primary identity of a client within the eD2k network and is used by other clients to track accumulated [credits](concepts.md).
...
> **Warning:** [`cryptkey.dat`](../manual/core/config-files/index.md#cryptkeydat) should be backed up and kept safe. Losing this file means losing your entire accumulated credit history with all other clients, because they cannot verify your identity without the matching public key.
...
| Step | Action |
|---|---|
| First start | aMule generates a 384-bit RSA key pair; private key saved to [`cryptkey.dat`](../manual/core/config-files/index.md#cryptkeydat) |
