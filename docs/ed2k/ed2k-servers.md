...
Static servers can be managed through the aMule GUI (right-click a server → Mark as static / Unmark as static) or by editing the [`staticservers.dat`](../manual/core/config-files/index.md#staticserversdat) file directly.
...
You can also add URLs to `~/.aMule/addresses.dat` (one per line) to have them checked at startup when auto-update is enabled — see the [`addresses.dat`](../manual/core/config-files/index.md#addressesdat) reference.
