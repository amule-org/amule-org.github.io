---
id: index
title: User Guide
---

This guide covers everything you need to use aMule: installation, configuration, daily usage, and reference documentation for components and files.

## New to aMule?

Follow these steps to get up and running:

1. **[Install aMule](./installation/index.md)** — get aMule on your system (Windows, macOS, Linux, or BSD).
2. **[Configure aMule](./core/index.md)** — set up your ports, bandwidth limits, and download folders.
3. **[Start using aMule](./interfaces/gui/usage/index.md)** — search for files, manage downloads, and share content.

## All Sections

| Section | Description |
|---|---|
| [Installation](./installation/index.md) | Installing aMule on Windows, macOS, Linux, and BSD |
| [Configuration](./core/index.md) | Preferences, ports, firewall, IP filter, and advanced options |
| [Usage](./interfaces/gui/usage/index.md) | Searching, downloading, uploading, and managing shared files |
| [aMule Components](./interfaces/index.md) | `amule`, `amuled`, `amulegui`, `amuleweb`, `amulecmd`, and utilities |
| [aMule Files](./core/config-files/index.md) | Configuration, data, and log files that aMule reads and writes |
| [Migration](./migration/index.md) | Migrating to aMule from eMule, eDonkey2000, or other eD2k clients |
