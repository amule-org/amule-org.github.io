...
:::note
Do not set `MaxConnections` above **1024** in [`amule.conf`](../core/files/amule-conf.md). The wxBase library that `amuled` uses cannot handle more simultaneous connections than that.
:::
...
Alternatively, run `amuled` once normally — it will create [`~/.aMule/amule.conf`](../core/files/amule-conf.md) with defaults — then quit and edit the file manually.

### Key [`amule.conf`](../core/files/amule-conf.md) Settings
