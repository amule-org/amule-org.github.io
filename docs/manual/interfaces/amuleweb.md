...
Wait a moment for [`~/.aMule/amule.conf`](../core/config-files/amule-conf.md) to be created, then stop amuled (Ctrl+C or `killall amuled`).

### 3. Edit [`~/.aMule/amule.conf`](../core/config-files/amule-conf.md)
...
The default skin name is `default`. Change it with the `Template` key in [`amule.conf`](../core/config-files/amule-conf.md) (`[WebServer]` section) or the `/Webserver/Template` key in [`remote.conf`](../core/config-files/remote-conf.md).
