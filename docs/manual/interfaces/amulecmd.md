...
Before using `amulecmd`, you must enable EC in aMule's Preferences (or in [`amule.conf`](../core/files/amule-conf.md)):
...
Alternatively, edit [`~/.aMule/amule.conf`](../core/files/amule-conf.md) directly:
...
### Generating [`remote.conf`](../core/files/remote-conf.md)

`amulecmd` stores its connection settings in [`~/.aMule/remote.conf`](../core/files/remote-conf.md). Generate it from the running aMule configuration:

```bash
amulecmd --create-config-from=/home/username/.aMule/amule.conf
```

This reads `ECPassword` and `ECPort` from [`amule.conf`](../core/files/amule-conf.md) and writes them to [`remote.conf`](../core/files/remote-conf.md).
...
You can also copy [`remote.conf`](../core/files/remote-conf.md) from the host machine and change the `Host=` line accordingly.
