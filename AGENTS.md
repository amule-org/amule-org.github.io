...
- **Image zoom**: content images are not click-to-zoom by default. Enable it for **large** images only — real width **≥ 850px** (wider than the ~750px content column, so they get downscaled); not icons/thumbnails or narrow dialogs. Use the HTML `<img>` form with the `enable-zoom` class: `<img src="/img/docs/<file>" alt="..." className="enable-zoom" />`.
